...
export default function DynamicInputList({ optionItems, setOptionItems }: { optionItems: Item[], setOptionItems: React.Dispatch<React.SetStateAction<Item[]>>, onListChange: () => void }) {

    // The array variable containing each typed value (requirement)
    const values = useMemo(() => optionItems.map((i) => i.value).join(","), [optionItems])
    ...
    const handleAdd = () => {
        ...
        setOptionItems((prev) => [...prev, { id: createId(), value: `Option ${nextNum}`, color: "228, 232, 237" }])
    }
    ...
    const onDropRow = (index: number, e: React.DragEvent) => {
        ...
        const fromIndex = Number.isNaN(Number(fromText)) ? dragFromIndexRef.current : Number(fromText)
        ...
        if (fromIndex !== adjustedDest) {
            setOptionItems((prev) => move(prev, fromIndex, Math.max(0, Math.min(prev.length, adjustedDest))))
        }
                ...
                {optionItems.map((item, index) => (
                    <li
                        ...
                        className={cn(
                            ...
                            draggingIndex === index && "scale-[0.99] opacity-80 shadow-lg rounded-xl",
                        )}

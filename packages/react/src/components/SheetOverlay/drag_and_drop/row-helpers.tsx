...
export const useRowDragAndDrop = (
  ...
  const computeInsertionFromPageY = (pageY: number) => {
    ...
    const localYInWorkbook = mouseYInWorkbook + context.scrollTop;
  ...
  const createGhost = (host: HTMLElement) => {
    ...
    el.style.position = "absolute";
    el.style.left = "8px";
    el.style.width = `${Math.max(60, context.rowHeaderWidth - 8)}px`;
    el.style.boxSizing = "border-box";
    el.style.padding = "6px 8px";
    el.style.background = "rgba(26,115,232,0.10)";
    el.style.border = "1px solid rgba(26,115,232,0.35)";
  ...
  const handleRowDrag = (ev: MouseEvent) => {
    ...
    const { lineTopPx, mouseYInWorkbook } = computeInsertionFromPageY(ev.pageY);
    ...
    if (dragRef.current.ghostEl) {
      const headerScroll = containerRef.current?.scrollTop ?? 0;
      dragRef.current.ghostEl.style.top = `${
        mouseYInWorkbook + headerScroll - dragRef.current.ghostHeightPx / 2
      }px`;
    }
  };
